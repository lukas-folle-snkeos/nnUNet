...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.log_file = join(self.output_folder, "training_log_%d_%d_%d_%02.0d_%02.0d_%02.0d.txt" %
                             (timestamp.year, timestamp.month, timestamp.day, timestamp.hour, timestamp.minute,
                              timestamp.second))
        if clear_ml_task is not None:
            self.task_logger = clear_ml_task.get_logger()
            self.logger = nnUNetLogger(clear_ml_logger=self.task_logger)
        else:
            self.task_logger: Logger = None
    ...
    def create_params_dict(self) -> Dict[str, Any]:
        params_dict = {
            'plans': self.plans_manager,
            'configuration': self.configuration_name,
            'fold': self.fold,
            'dataset_json': self.dataset_json,
            'unpack_dataset': self.unpack_dataset,
            'device': self.device,
            'clear_ml_task': self.task_logger,
            'is_ddp': self.is_ddp,
            'local_rank': self.local_rank,
            'preprocessed_dataset_folder_base': self.preprocessed_dataset_folder_base,
    ...
    def print_params(self):
        params_dict = self.create_params_dict()
